...
# ---------------------------
file_pattern = "data/part2/momentum/training_stats_*.dat"
...
for file_path in files:
    ...
    tokens = base_name.replace("_momentum.dat", "").split('_')
    if len(tokens) < 4:
...
plt.title("Training Accuracy vs Iterations (Momentum)")
plt.grid(True)
plt.legend(loc='best')
plt.tight_layout()
plt.savefig("data/part2/momentum/accuracy_training_plot_momentum.png")
plt.close()
...
plt.ylabel("Test Accuracy")
plt.ylim(0.85, 1)
plt.title("Test Accuracy (Iterations > 30000) (Momentum)")
plt.grid(True)
plt.legend(loc='best')
plt.tight_layout()
plt.savefig("data/part2/momentum/accuracy_training_plot_momentum_post_30000.png")
plt.close()
