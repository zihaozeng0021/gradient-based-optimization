...
# ---------------------------
file_pattern = "data/part2/learning_rate_decay/training_stats_*.dat"
...
for file_path in files:
    ...
    tokens = base_name.replace("lr_decay.dat", "").split('_')
    if len(tokens) < 4:
...
plt.title("Training Accuracy vs Iterations (LR Decay)")
plt.grid(True)
plt.legend(loc='best')
plt.tight_layout()
plt.savefig("data/part2/learning_rate_decay/accuracy_training_plot_lr_decay.png")
plt.close()
...
plt.ylabel("Test Accuracy")
plt.ylim(0.85, 1)
plt.title("Test Accuracy (Iterations > 30000) (LR Decay)")
plt.grid(True)
plt.legend(loc='best')
plt.tight_layout()
plt.savefig("data/part2/learning_rate_decay/accuracy_after_30000_plot_lr_decay.png")
plt.close()
