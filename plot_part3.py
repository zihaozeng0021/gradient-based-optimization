...
# ---------------------------
file_pattern = "data/part3/training_stats_*.dat"
...
for file_path in files:
    ...
    tokens = base_name.replace(".dat", "").split('_')
    if len(tokens) < 4:
...
plt.title("Training Accuracy vs Iterations (Adam)")
plt.grid(True)
plt.legend(loc='best')
plt.tight_layout()
plt.savefig("data/part3/accuracy_training_plot.png")
plt.close()
...
plt.ylabel("Test Accuracy")
plt.ylim(0.85, 1)
plt.title("Test Accuracy (Iterations > 30000) (Adam)")
plt.grid(True)
plt.legend(loc='best')
plt.tight_layout()
plt.savefig("data/part3/accuracy_training_plot_post_30000.png")
plt.close()
